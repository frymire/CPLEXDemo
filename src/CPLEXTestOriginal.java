...
public class CPLEXTestOriginal {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if ( cplex.solve() ) {
				...
				int nvars = x.length;
				for (int j = 0; j < nvars; ++j) cplex.output().println("Variable " + j + ": Value = " + x[j] + " Reduced cost = " + dj[j]);

				int ncons = slack.length;
				for (int i = 0; i < ncons; ++i) cplex.output().println("Constraint " + i + ": Slack = " + slack[i] + " Pi = " + pi[i]);
